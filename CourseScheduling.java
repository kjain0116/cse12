...
public class CourseScheduling {
    ...
    public static Student getStudent(String pid) {
        ...
        while(iter.hasNext()) {
        	...
        	if(student.getStudentID().equals(pid))
        		return student;
        }
        return null;
        ...
        try {
            ...
            while (scLine.hasNextLine()) {//if the file has next line
                ...
                if (property.equals("register")) {
                	...
                	if(getStudent(pid).getmyEnrolledCourses().contains(getCourse(code))) {
                		printFail(getStudent(pid), getCourse(code), true);
                	}
                	else if(getStudent(pid).getmyWaitlist().contains(getCourse(code))) {
                		printFail(getStudent(pid), getCourse(code), false);
                	}
                	else if(getStudent(pid).getCoins() < coins) {
                		printNoCoins(getStudent(pid), getCourse(code));	
                	}
                	else {
                		Registration registration = new Registration( getStudent(pid), getCourse(code), coins);
                    	getCourse(code).addToWaitlist(registration);
                    	getStudent(pid).deductCoins(coins);
                    	print(getStudent(pid), getCourse(code), coins, false);
                	}
                    ...
                    for(int j = 0; j< num; j++) {
                    	for(int i = 0; i < courseList.size(); i++) {
                    		if(courseList.get(i).isFull()) {
                    			...
                    			Registration r = courseList.get(i).processWaitlist();
                    			if(r != null)
                    				print(r.getStudent(), courseList.get(i), r.getCoins(), true);
                    		}
                    	}
